...
-   [#755](https://github.com/stripe/stripe-php/pull/755) Respect Checkout-Should-Retry and Retry-After headers
...
-   Use `\Checkout\<class>::class` constant instead of strings ([#643](https://github.com/stripe/stripe-php/pull/643))
...
-   Update constructors of Checkout exception classes ([#559](https://github.com/stripe/stripe-php/pull/559))
...
-   Add `ErrorObject` to Checkout exceptions ([#705](https://github.com/stripe/stripe-php/pull/705))
...
-   [#539](https://github.com/stripe/stripe-php/pull/539) Fix methods on `\Checkout\PaymentIntent` to properly pass arguments to the API.

## 6.19.4 - 2018-10-11

-   [#534](https://github.com/stripe/stripe-php/pull/534) Fix PSR-4 autoloading for `\Checkout\FileUpload` class alias

## 6.19.3 - 2018-10-09

-   [#530](https://github.com/stripe/stripe-php/pull/530) Add constants for `flow` (`FLOW_*`), `status` (`STATUS_*`) and `usage` (`USAGE_*`) on `\Checkout\Source`
...
-   [#523](https://github.com/stripe/stripe-php/pull/523) Add support for Checkout Terminal

## 6.18.0 - 2018-09-24

-   [#520](https://github.com/stripe/stripe-php/pull/520) Rename `\Checkout\FileUpload` to `\Checkout\File`
...
-   [#502](https://github.com/stripe/stripe-php/pull/502) Add `isDeleted()` method to `\Checkout\StripeObject`

## 6.12.0 - 2018-07-28

-   [#501](https://github.com/stripe/stripe-php/pull/501) Add support for scheduled query runs (`\Checkout\Sigma\ScheduledQueryRun`) for Sigma

## 6.11.0 - 2018-07-26

-   [#500](https://github.com/stripe/stripe-php/pull/500) Add support for Checkout Issuing

## 6.10.4 - 2018-07-19

-   [#498](https://github.com/stripe/stripe-php/pull/498) Internal improvements to the `\Checkout\ApiResource.classUrl()` method
...
-   [#493](https://github.com/stripe/stripe-php/pull/493) Add PHPDoc for `auto_advance` on `\Checkout\Invoice`

## 6.10.0 - 2018-06-28

-   [#488](https://github.com/stripe/stripe-php/pull/488) Add support for `$appPartnerId` to `Checkout::setAppInfo()`
...
-   [#481](https://github.com/stripe/stripe-php/pull/481) Add new `\Checkout\Discount` and `\Checkout\OrderItem` classes, add more PHPDoc describing object attributes
...
-   [#479](https://github.com/stripe/stripe-php/pull/479) Fix unnecessary traits on `\Checkout\InvoiceLineItem`

## 6.7.2 - 2018-05-28

-   [#471](https://github.com/stripe/stripe-php/pull/471) Add `OBJECT_NAME` constant to all API resource classes, add `\Checkout\InvoiceLineItem` class
...
-   [#458](https://github.com/stripe/stripe-php/pull/458) Add PHPDoc for `account` on `\Checkout\Event`
...
-   [#453](https://github.com/stripe/stripe-php/pull/453) Add constants for `reason` (`REASON_*`) and `status` (`STATUS_*`) on `\Checkout\Dispute`

## 6.3.2 - 2018-02-27

-   [#452](https://github.com/stripe/stripe-php/pull/452) Add PHPDoc for `amount_paid` and `amount_remaining` on `\Checkout\Invoice`

## 6.3.1 - 2018-02-26

-   [#443](https://github.com/stripe/stripe-php/pull/443) Add event types as constants to `\Checkout\Event` class

## 6.3.0 - 2018-02-23

-   [#450](https://github.com/stripe/stripe-php/pull/450) Add support for `code` attribute on all Checkout exceptions

## 6.2.0 - 2018-02-21

-   [#440](https://github.com/stripe/stripe-php/pull/440) Add support for topups
-   [#442](https://github.com/stripe/stripe-php/pull/442) Fix PHPDoc for `\Checkout\Error\SignatureVerification`
...
*   `\Checkout\AttachedObject` no longer exists. Attributes that used to be instances of `\Checkout\AttachedObject` (such as `metadata`) are now instances of `\Checkout\StripeObject`.

-   Attributes that used to be PHP arrays (such as `legal_entity->additional_owners` on `\Checkout\Account` instances) are now instances of `\Checkout\StripeObject`, except when they are empty. `\Checkout\StripeObject` has array semantics so this should not be an issue unless you are actively checking types.

*   `\Checkout\Collection` now derives from `\Checkout\StripeObject` rather than from `\Checkout\ApiResource`.
...
-   Add `\Checkout\Checkout::setLogger()` to support an external PSR-3 compatible logger
...
-   Add `Checkout::setAppInfo` to allow plugins to register user agent information
...
-   Allow global account ID to be set with `Checkout::setAccountId()`
...
-   Rename `\Checkout\Object` to `\Checkout\StripeObject` (PHP 7 compatibility)
-   Rename `getCode` and `getParam` in exceptions to `getStripeCode` and `getStripeParam`
-   Add support for calling `json_encode` on Checkout objects in PHP 5.4+
-   Start supporting/testing PHP 7

## 2.3.0 - 2015-07-06

-   Add request ID to all Checkout exceptions
...
-   Switched to Checkout namespace instead of Checkout\_ class name prefiexes (thanks @chadicus!)
...
-   Add support for multiple cards API (Checkout API version 2013-07-12: https://stripe.com/docs/upgrades#2013-07-05)
...
-   Ensure that all input is encoded in UTF-8 before submitting it to Checkout's servers. (github issue #27)

## 1.7.13 - 2013-02-01

-   Add support for passing options when retrieving Checkout objects e.g., Stripe_Charge::retrieve(array("id"=>"foo", "expand" => array("customer"))); Stripe_Charge::retrieve("foo") will continue to work

## 1.7.12 - 2013-01-15

-   Add support for setting a Checkout API version override
...
-   Bugfix so that Checkout objects (e.g. Customer, Charge objects) used in API calls are transparently converted to their object IDs
...
-   Make clearer that you should be including lib/Checkout.php, not test/Checkout.php (github issue #14)
...
-   Switch from using HTTP Basic auth to Bearer auth. (Note: Checkout will support Basic auth for the indefinite future, but recommends Bearer auth when possible going forward)
