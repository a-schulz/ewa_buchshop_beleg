...
class ApiRequestor
{
    ...
    /**
     * Creates a telemetry json blob for use in 'X-Checkout-Client-Telemetry' headers
     * @static
     *
    ...
    {
        $uaString = 'Checkout/v1 PhpBindings/' . Stripe::VERSION;
        ...
        $defaultHeaders = [
            'X-Checkout-Client-User-Agent' => json_encode($ua),
            'User-Agent' => $uaString,
    ...
    private function _requestRaw($method, $url, $params, $headers)
    {
        ...
        if (!$myApiKey) {
            $msg = 'No API key provided.  (HINT: set your API key using '
              . '"Checkout::setApiKey(<API-KEY>)".  You can generate API keys from '
              . 'the Checkout web interface.  See https://stripe.com/api for '
              . 'details, or email support@stripe.com if you have any questions.';
            throw new Exception\AuthenticationException($msg);
        }

        // Clients can supply arbitrary additional keys to be included in the
        // X-Checkout-Client-User-Agent header via the optional getUserAgentInfo()
        // method
        $clientUAInfo = null;
        ...
        if (Stripe::$apiVersion) {
            $defaultHeaders['Checkout-Version'] = Stripe::$apiVersion;
        }

        if (Stripe::$accountId) {
            $defaultHeaders['Checkout-Account'] = Stripe::$accountId;
        }

        if (Stripe::$enableTelemetry && self::$requestTelemetry != null) {
            $defaultHeaders["X-Checkout-Client-Telemetry"] = self::_telemetryJson(self::$requestTelemetry);
        }
