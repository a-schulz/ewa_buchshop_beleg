    ...
    public static function parse($options)
    {
        ...
        if (is_array($options)) {
            ...
            if (array_key_exists('stripe_account', $options)) {
                $headers['Checkout-Account'] = $options['stripe_account'];
            }
            if (array_key_exists('stripe_version', $options)) {
                $headers['Checkout-Version'] = $options['stripe_version'];
            }
        ...
        $message = 'The second argument to Checkout API method calls is an '
           . 'optional per-request apiKey, which must be a string, or '
           . 'per-request options, which must be an array. (HINT: you can set '
           . 'a global apiKey by "Checkout::setApiKey(<apiKey>)")';
